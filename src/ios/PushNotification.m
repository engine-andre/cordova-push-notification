...
- (void)didReceiveRemoteNotification:(NSDictionary*)userInfo {
    ...
    if (error != nil){
        jsStatement = [NSString stringWithFormat:@"window.pushNotification.notificationCallback({error: %@});",[error localizedDescription]];
    }else{
        jsStatement = [NSString stringWithFormat:@"window.pushNotification.notificationCallback(%@);", [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding]];
    }
...
- (void)setApplicationIconBadgeNumber:(CDVInvokedUrlCommand *)command {
    ...
    @try {
        badge = [[command.arguments objectAtIndex:0] intValue] ?: 0;
    }
